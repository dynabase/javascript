...
- If you don't have any special reason to use a class component like some lifecycle methods dont use `class extends React.Component` or `React.createClass`. eslint: [`react/prefer-es6-class`](https://github.com/yannickcr/eslint-plugin-react/blob/master/docs/rules/prefer-es6-class.md) [`react/prefer-stateless-function`](https://github.com/yannickcr/eslint-plugin-react/blob/master/docs/rules/prefer-stateless-function.md)
...
- Ordering for Functional Components `React.FC`:

1. Local state `useState` hooks
1. Other hooks (custom-made or from external libraries)
1. Local `useEffect` hooks
1. _clickHandlers or eventHandlers_ like `onClickSubmit()` or `onChangeDescription()`
1. _getter methods for `return`_ like `getSelectReason()` or `getFooterContent()`
1. `return`

> Note: The ordering of the hooks might change if you want to compose your hooks logically. But we recommend to stick to the order.
